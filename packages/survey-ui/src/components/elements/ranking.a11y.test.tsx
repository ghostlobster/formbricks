...
describe("Ranking accessibility", () => {
  it("wraps options in a fieldset with sr-only legend", () => {
  ...
  it("unranked option buttons have descriptive aria-label", () => {
    const { container } = render(<Ranking {...defaultProps} />);
  ...
  it("ranked items have Move up / Move down buttons with descriptive labels", () => {
    const { container } = render(
      <Ranking {...defaultProps} value={["opt1", "opt2", "opt3"]} />
    );
  ...
  it("has no axe violations in unranked state", async () => {
    const { container } = render(<Ranking {...defaultProps} />);
    const violations = await runAxe(container);
    expect(violations, formatViolations(violations)).toHaveLength(0);
  });

  it("has no axe violations with items ranked", async () => {
    const { container } = render(
      <Ranking {...defaultProps} value={["opt1", "opt2", "opt3"]} />
    );
    const violations = await runAxe(container);
